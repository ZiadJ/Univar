    ...
    public static partial class Storage
    {
        public static class User
        {
            ...
            public static bool IsAuthenticated
            {
                get
                {
                    return (User.HttpContext.User != null
                        && User.HttpContext.User.Identity != null
                        && !string.IsNullOrEmpty(User.HttpContext.User.Identity.Name));
                }
            }
            ...
            public static string Name
            {
                get
                {
                    ...
                    {
                        return User.HttpContext.User.Identity.Name;
                    }
                    else
                    {
                        return !string.IsNullOrEmpty(User.HttpContext.Profile.UserName) ? User.HttpContext.Profile.UserName : Storage.Cookie.Get("Userkey");
                    }
                }
                set
                {
            ...
            {
                if (!User.HttpContext.Profile.IsAnonymous)
                    return Membership.GetUser(User.HttpContext.Profile.UserName).ProviderUserKey.ToString();
                else
                    return null;
            }
            ...
            public static string GetSessionID()
            {
                ...
                if (User.HttpContext.Session.IsNewSession)
                    User.HttpContext.Session["init"] = 0;

                return User.HttpContext.Session.SessionID;
            }
            ...
            {
                User.HttpContext.Response.Redirect(User.HttpContext.Request.Url.PathAndQuery, false);
            }
        }
    }
}
